...
def make_report(config):
    ...
    group_by_feature = config['group_by_feature']
    ...
    results_directory = f'reports/pred_{label_column}' \
                        f'_with_separation_per_{group_by_feature}' \
    ...
    grouped_df = df.groupby(group_by_feature)
    for group_name, group_df in grouped_df:
        lines_to_save = []
        group_directory = f'{results_directory}/{group_by_feature}_{group_name}'
        os.mkdir(group_directory)

        X, y = group_df.drop(columns=label_column), group_df[label_column]
        ...
        plt.savefig(f'{group_directory}/confusion_matrix')
        ...
        plt.savefig(f'{group_directory}/box_plot_important_features')
        mpld3.save_html(fig, f'{group_directory}/box_plot_important_features.html')

        # Open a text file in write mode
        with open(f'{group_directory}/results.txt', "w") as file:
...
config = {
    ...
    'group_by_feature': 'ACCR',  # separate data according to this feature
    ...
    'unfair_features': ['meanF0Hz', 'stdevF0Hz', 'File_name', 'SUBJ', 'TRLN', 'BLKN',
...
file_options = ['csv_files/data_csv_removed_high_correlation_harsher.csv']
