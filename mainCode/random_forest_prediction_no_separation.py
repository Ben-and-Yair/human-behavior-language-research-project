"""Reads the given configurations (and iterates over a set of options for parameters), creates a random forest
classifier, get the most important features and removes a portion of the least important ones. We repeat this
process until the number of features hits a certain threshold. All the results (and the parameters chosen) are
...
def normalize_per_subject_per_session(df, features, norm_by_first_sess=False):
    """splitting the data to groups based on both SUBJ and SESS (meaning there are
    len(SESS)*len(SUBJ) groups), then normalising each feature in each group,
    if norm_by_first_sess=True, then normalise each session according to the mean
    of the first session"""
    df_copy = df.copy()
    grouped_subject = df_copy.groupby('SUBJ')
    for subject, subject_group in grouped_subject:
        grouped_session = subject_group.groupby('SESS')
        update_mean = len(grouped_session.groups)  # number of times we update the mean
        if norm_by_first_sess:
            update_mean = 1
        mean = 0
...
def run_random_forest_with_feature_selection(X_train, y_train, X_test, y_test, output_addition,
                                             ...
                                             max_tree_depth=None):
    """Receives the training and validation sets, and some RF parameters, trains an RF model for
    feature selection and one for testing accuracy, then returns the features sorted by importance
...
def plot_feature_boxplot(df_all_classes, important_features, title, target_label):
    """Receives a df of the selected classes and a list of desired fetures, then
    returns a box plot of the features according to the df"""
    sns.set(style="ticks")
...
def make_report(config):
    """Receives a dictionary of parameters and iterates over the random forset training until
    a threshold of remaining features is met. The results are saved to a directory named
    reports/pred_<determined by parameters>"""
    path_to_data = config['PATH_TO_DATA']
    label_column = config['label_column']
    possible_values = config['possible_values']
    remove_session_f = config['remove_session_2_3']
    smote_flag = config['smote_flag']
    add_relative_durations = config['add_relative_durations']
    ...
    results_directory = f'../reports/pred_{label_column}_no_separation' \
                        ...
                        f'_with{relative_dur}_relative_duration'
    ...
    # adding relative duration
    if add_relative_durations:
        normalize_per_subject_per_session(df, 'Total_duration(s)', norm_by_first_sess=True)
...
# --- Configurations ---
# edit this config dictionary to match your need
config = {
    ...
    'junk_features': ['Unnamed: 0.1', 'Unnamed: 0'],  # 'Unnamed: 0.2', 'cut_WORD', 'robot_time', 'total_time'
...
# ---------------------------------------------- #
def main():
    # these options here are overriding the "config" dictionary in order to iterate
    # over multiple option without requiring several reruns of the code
    file_options = ['../csv_files/segments_features_and_all_features_with_participants_logs.csv']
